...
async function main() {
  ...
  try {
    ...
    assert.equal(effectiveCodex.args[0], "exec");
    assert.ok(effectiveCodex.args.includes("--skip-git-repo-check"));
    // Regresyon: `codex exec` varsayilan read-only sandbox'ta dosya YAZAMAZ ama surec 0 ile
    // biter; motor delegasyonu "tamamlandi" sanip tur ustune tur harcardi. Yazma izni sart.
    assert.deepEqual(
      effectiveCodex.args.slice(effectiveCodex.args.indexOf("--sandbox"), effectiveCodex.args.indexOf("--sandbox") + 2),
      ["--sandbox", "workspace-write"]);
    // Kullanicinin kendi sandbox/onay tercihi ezilmemeli.
    for (const own of [["exec", "--sandbox", "danger-full-access"], ["exec", "--full-auto"]]) {
      const custom = cliRegistry.effectiveAgent({ cmd: "codex", args: own.slice() });
      assert.ok(!custom.args.includes("workspace-write"), `${own.join(" ")} icin sandbox tercihi ezilmemeli`);
    }
